...
static int32_t	raw_stm32			= 0;					// Previous value of the MCU temperature (RAW)
static int		cached_stm32		= 0;					// Previous value of the MCU temperature
	...
	if (noAmbientSensor()) {								// No T12 IRON handle is connected, calculate MCU internal temperature
		if (abs(t_stm32.read() - raw_stm32) < 20)
			return cached_stm32;
	...
	if (abs(t_amb.read() - raw_ambient) < 20)
