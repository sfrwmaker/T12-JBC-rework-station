...
bool BRGT::adjust(void) {
	...
	if (brightness > TFT_TIM.Instance->CCR1) {
		if (brightness - TFT_TIM.Instance->CCR1 > b_step) {
			TFT_TIM.Instance->CCR1 += b_step;
		} else {
			++TFT_TIM.Instance->CCR1;
		}
		return true;
	} else if (brightness < TFT_TIM.Instance->CCR1) {
		if (TFT_TIM.Instance->CCR1 - brightness > b_step) {
			TFT_TIM.Instance->CCR1 -= b_step;
		} else {
			--TFT_TIM.Instance->CCR1;
		}
...
void DSPL::debugShow(uint16_t data[12], bool t12_on, bool jbc_on, bool gun_on, bool t12_connected, bool jbc_connected, bool gun_connected, bool gun_reed, bool jbc_stby, bool jbc_change, bool gtim_ok) {
	...
	for (uint8_t i = 0; i < 6; ++i) {									// Draw 6 lines of two columns
		...
		if ((i == 0 && t12_on) || (i == 4 && !gtim_ok)) clr = gd_color;
	...
	sprintf(buff, "[%c.%c.%c]", jbc_stby?'s':' ', jbc_change?'c':' ', gun_reed?'r':' ');
	strToBitmap(bm, buff, align_center);
	drawBitmap(10, top+6*h, bm, bg_color, fg_color);
	bm.clear();
	sprintf(buff, "(%c-%c-%c)", t12_connected?'t':' ', jbc_connected?'j':' ', gun_connected?'g':' ');
	strToBitmap(bm, buff, align_center);
