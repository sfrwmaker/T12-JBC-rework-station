...
class CFG_CORE: public TIPS {
	public:
		...
		uint8_t		getOffTimeout(tDevice dev) 			{ if (dev == d_jbc) return a_cfg.jbc_off_timeout; else return a_cfg.t12_off_timeout;				}
		uint16_t	getLowTemp(tDevice dev)				{ if (dev == d_jbc) return a_cfg.jbc_low_temp;    else return a_cfg.t12_low_temp; 					}
		...
		void		setup(bool buzzer, bool celsius, bool big_temp_step, bool i_enc, bool g_enc, uint8_t bright);
